...
class InsertTagsListenerTest extends TestCase
{
    ...
    public function testOnInsertFlagsFileUrl()
    {
        ...
        $file->method('findByUuid')->willReturn($model);

        $environment = $this->createAdapterMock(['get']);
        $environment->method('get')->willReturn('http://domain.tld/');
     ...
     */
    public function testOnInsertFlagsEventUrl($provided, $expected)
    {
        $listener = $this
        ...
        $listener->method('classExists')->willReturn($provided[1]);

        $this->assertEquals($expected, $listener->onInsertTagFlags('absolute', 'event_url::1', ''));
    }
    ...
    public function eventUrlDataProvider()
    {
        ...
        $eventAdapter1->method('findByIdOrAlias')->willReturn(null);
        ...
        $eventAdapter2->method('findByIdOrAlias')->willReturn($event2);
        ...
        $eventAdapter3->method('findByIdOrAlias')->willReturn($event3);

        $page3 = $this->createAdapterMock(['getAbsoluteUrl']);
        $page3->type = '';
        $page3->method('getAbsoluteUrl')->willReturn('http://domain.tld/internal.html');

        $pageAdapter3 = $this->createAdapterMock(['findByIdOrAlias']);
        $pageAdapter3->method('findByIdOrAlias')->willReturn($page3);
        ...
        $eventAdapter4->method('findByIdOrAlias')->willReturn($event4);

        $page4 = $this->createAdapterMock(['getAbsoluteUrl']);
        $page4->method('getAbsoluteUrl')->willReturn('http://domain.tld/articles/foobar.html');

        $article4 = $this->createAdapterMock(['getRelated']);
        $article4->alias = 'foobar';
        $article4->method('getRelated')->willReturn($page4);

        $articleAdapter4 = $this->createAdapterMock(['findByIdOrAlias']);
        $articleAdapter4->method('findByIdOrAlias')->willReturn($article4);

        // Regular event
        $page5 = $this->createAdapterMock(['getAbsoluteUrl']);
        $page5->method('getAbsoluteUrl')->willReturn('http://domain.tld/events/foobar.html');

        $calendar5 = $this->createAdapterMock(['getRelated']);
        $calendar5->method('getRelated')->willReturn($page5);

        $event5 = $this->createAdapterMock(['getRelated']);
        $event5->source = '';
        $event5->alias = 'foobar';
        $event5->method('getRelated')->willReturn($calendar5);

        $eventAdapter5 = $this->createAdapterMock(['findByIdOrAlias']);
        $eventAdapter5->method('findByIdOrAlias')->willReturn($event5);

        // No calendar
        $event6 = $this->createAdapterMock(['getRelated']);
        $event6->source = '';
        $event6->method('getRelated')->willReturn(null);

        $eventAdapter6 = $this->createAdapterMock(['findByIdOrAlias']);
        $eventAdapter6->method('findByIdOrAlias')->willReturn($event6);
     ...
     */
    public function testOnInsertFlagsFaqUrl($provided, $expected)
    {
        $listener = $this
            ->getMockBuilder(InsertTagsListener::class)
            ->setConstructorArgs([$this->createFrameworkMock($provided[0])])
        ...
        $this->assertEquals($expected, $listener->onInsertTagFlags('absolute', 'faq_url::1', ''));
    }
    ...
    public function faqUrlDataProvider()
    {
        ...
        $faqAdapter1->method('findByIdOrAlias')->willReturn(null);

        // No category
        $faq2 = $this->createAdapterMock(['getRelated']);
        $faq2->method('getRelated')->willReturn(null);

        $faqAdapter2 = $this->createAdapterMock(['findByIdOrAlias']);
        $faqAdapter2->method('findByIdOrAlias')->willReturn($faq2);

        // No page
        $category3 = $this->createAdapterMock(['getRelated']);
        $category3->method('getRelated')->willReturn(null);

        $faq3 = $this->createAdapterMock(['getRelated']);
        $faq3->method('getRelated')->willReturn($category3);

        $faqAdapter3 = $this->createAdapterMock(['findByIdOrAlias']);
        $faqAdapter3->method('findByIdOrAlias')->willReturn($faq3);

        // Regular faq
        $page4 = $this->createAdapterMock(['getAbsoluteUrl']);
        $page4->method('getAbsoluteUrl')->willReturn('http://domain.tld/items/faq.html');

        $category4 = $this->createAdapterMock(['getRelated']);
        $category4->method('getRelated')->willReturn($page4);

        $faq4 = $this->createAdapterMock(['getRelated']);
        $faq4->alias = 'foobar';
        $faq4->method('getRelated')->willReturn($category4);

        $faqAdapter4 = $this->createAdapterMock(['findByIdOrAlias']);
        $faqAdapter4->method('findByIdOrAlias')->willReturn($faq4);
     ...
     */
    public function testOnInsertFlagsNewsUrl($provided, $expected)
    {
        $listener = $this
            ->getMockBuilder(InsertTagsListener::class)
            ->setConstructorArgs([$this->createFrameworkMock($provided[0])])
        ...
        $this->assertEquals($expected, $listener->onInsertTagFlags('absolute', 'news_url::1', ''));
    }
    ...
    public function newsUrlDataProvider()
    {
        ...
        $newsAdapter1->method('findByIdOrAlias')->willReturn(null);
        ...
        $newsAdapter2->method('findByIdOrAlias')->willReturn($news2);
        ...
        $newsAdapter3->method('findByIdOrAlias')->willReturn($news3);
        ...
        $newsAdapter4->method('findByIdOrAlias')->willReturn($news4);
        ...
        // Regular news
        $page5 = $this->createAdapterMock(['getAbsoluteUrl']);
        $page5->method('getAbsoluteUrl')->willReturn('http://domain.tld/newss/foobar.html');

        $archive5 = $this->createAdapterMock(['getRelated']);
        $archive5->method('getRelated')->willReturn($page5);

        $news5 = $this->createAdapterMock(['getRelated']);
        $news5->source = '';
        $news5->alias = 'foobar';
        $news5->method('getRelated')->willReturn($archive5);

        $newsAdapter5 = $this->createAdapterMock(['findByIdOrAlias']);
        $newsAdapter5->method('findByIdOrAlias')->willReturn($news5);

        // No calendar
        $news6 = $this->createAdapterMock(['getRelated']);
        $news6->source = '';
        $news6->method('getRelated')->willReturn(null);

        $newsAdapter6 = $this->createAdapterMock(['findByIdOrAlias']);
        $newsAdapter6->method('findByIdOrAlias')->willReturn($news6);
